/**
 * Server copy of client TerrainSurface — must stay identical for flag spawn vs player altitude.
 * Radial displacement vs the base sphere radius (matches Globe / Carpet / Plane).
 */
import {
  createNoise3D,
  sampleTerrainValue,
...
} from "./SimplexNoise.js";

/** Same as Three.js MathUtils.smoothstep(x, min, max). */
function smoothstepX(x: number, min: number, max: number): number {
  if (x <= min) return 0;
  if (x >= max) return 1;
...
function landDisplacement(
  ...
  const peakMask = Math.pow(smoothstepX(elevation, 0.52, 0.86), 1.35);
