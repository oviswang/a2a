...
export class Game {
  ...
  private computeNextStepHint(): string {
    ...
    if (this.braziers && this.braziers.placedCount > 0 && !this.braziers.allFiveEternalAndLit()) {
      ...
      return `${lead}To stop the moon for good you need eternal flames, and only the MAGIC CARPET can enter the cosmic-void portals where they're earned — switch to the carpet from the lobby/menu, fly into a void portal, survive the moth waves, then plant each flame at a brazier. ${progress}`;
    }
    if (moonClose) {
