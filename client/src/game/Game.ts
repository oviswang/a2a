...
/** Two carpets within this chord distance trigger the shared photo keepsake. */
const CARPET_PHOTO_RANGE = 0.6;
/** Per-peer cooldown before another photo moment with the same carpet. */
const CARPET_PHOTO_COOLDOWN_MS = 90_000;
/** Global gap so a crowd of carpets doesn't spam photo moments. */
const CARPET_PHOTO_GLOBAL_GAP_MS = 12_000;
...
/** A nearby pilot's observable activity, for context-aware greetings. */
...
export class Game {
  ...
  private fishComboCount = 0;
  private fishComboExpiresAt = 0;
  ...
  private installDiag() {
    ...
    try {
      const api: Record<string, unknown> = {
        ...
        /** The full persistent player profile pushed to the companion as
  ...
  private buildCompanionManager(vehicle: Vehicle | null, token: string): CompanionManager {
    ...
    // Pin the reply language to the player's app language (their browser locale) so
    // an English player always gets an English co-pilot and a Chinese player a
    // Chinese one — regardless of the agent's dashboard default. Drives text replies
    // AND the call opener (appContext grounds both). Voice TTS also gets locale via
    // connectCall below.
    const langDirective = IS_ZH
      ? "LANGUAGE — 这位玩家的应用语言是中文。请始终用简体中文和 TA 说话与打字（除非 TA 自己明确改用别的语言）。"
      : "LANGUAGE — this player's app language is English. Always speak and write to them in English (unless they clearly switch languages themselves). ";
    ...
    const manager = new CompanionManager({
      ...
      tools: [
        ...
        {
          name: "gift_eternal_flame",
          description:
            "Give ONE of your earned Eternal Flames to the companion-pilot you just met (game.event.met_companion). Only works if you currently hold an Eternal Flame (game.situation eternalFlames > 0) — those are earned in the cosmic void on the magic carpet. It's a generous gift so a teammate (even on a plane or boat, who can't earn flames) can plant it at a brazier toward the shared win; it earns a big friendship bond. Use when the player wants to help a teammate finish the braziers, or offers to share a flame.",
          parameters: { type: "object", properties: {} },
        },
        {
        ...
        },
        {
          name: "open_guide",
          description:
            "Open the full how-to-play web guide for the player (a friendly illustrated page covering vehicles, the eternal-flame win path, activities, and the A2A co-op features). It opens in the player's language automatically (English by default, Chinese for Chinese browsers). Use this when the player is new, confused, asks 'how do I play / what do I do', can't figure out a mechanic (e.g. how to stop the moon, how the void works, how to save the world together), or explicitly asks for help or a guide — it's a gentle way to hand them the manual. You can also just mention the URL in chat if that fits better.",
          parameters: { type: "object", properties: {} },
        },
      ],
  ...
  private async execCompanionTool(
    ...
    if (name === "gift_eternal_flame") {
      return this.giftEternalFlame();
    }
    ...
    if (name === "open_guide") {
      // Open the how-to-play web guide in the player's language (default English;
      // Chinese for Chinese browsers). The page also auto-detects, but pin ?lang so
      // the link the companion shares always matches the player.
      const lang = IS_ZH ? "zh" : "en";
      const url = `${location.origin}/guide.html?lang=${lang}`;
      try {
        window.open(url, "_blank", "noopener");
      } catch {
        /* popup blocked — the URL is still returned for the companion to share in chat */
      }
      this.hud.showAmbientToast(
        t("📖 Opened the player guide", "📖 已打开玩法指南"),
        3000,
      );
      return { ok: true, result: `Opened the player guide for the player: ${url}` };
    }
  ...
  private detectCompanionEncounters(localWorldPos: Vector3) {
    ...
    this.remotePlanes.forEachRemote((p) => {
      ...
      const activity: GreetActivity = { vehicle: p.vehicleType, carrying: p.isCarrying };
      if (!p.companionName) {
      ...
      if (!nearest || d < nearest.d) nearest = { id: p.id, name: p.name, companionName: p.companionName, d, activity };
    });
    ...
    if (!nearest) return;
    const near = nearest as { id: string; name: string; companionName: string | null; d: number; activity: GreetActivity };
    ...
    this.onCompanionPilotEncounter(near.id, near.name, near.companionName, near.activity);
  }

  /** Two companion-pilots just met — set the greet target, let our own agent react,
   *  surface a one-tap "say hi / pair" chip, AND have our companion open with a line
   *  so the two agents naturally chatter (the peer's agent may reply). */
  private onCompanionPilotEncounter(
    socketId: string,
    name: string,
    companionName: string | null,
    ...
    // Group chatter: open with a line (throttled by the shared global gap) so the
    // two companions visibly talk; the peer's agent can reply via companion_hailed.
    if (Date.now() - this.lastProactiveGreetAt >= Game.GREET_GLOBAL_GAP_MS) {
      this.lastProactiveGreetAt = Date.now();
      void this.sendProactiveGreeting(socketId, name, activity, true);
    }
  }
  ...
  private trackProactiveGreet(socketId: string, name: string, activity: GreetActivity, d: number, now: number) {
    ...
    if (
      ...
      void this.sendProactiveGreeting(socketId, name, activity, false);
    }
    this.greetEncounters.set(socketId, st);
  }

  /** Send a one-line greeting to a co-present pilot. Tries a REAL AI-composed line
   *  (the agent grounds on the owner's live situation + the recipient's activity);
   *  falls back to a localized template if the agent is busy/unavailable. A non-AI
   *  recipient receives it passively as a speech bubble. */
  private async sendProactiveGreeting(
    ...
    let line = await this.companion.composeLine(this.buildGreetingPrompt(name, activity, peerHasAI));
    line = line ? this.sanitizeGreetLine(line) : null;
    if (!line) line = this.composeProactiveGreeting(name, activity, peerHasAI);
    if (!this.socketClient) return; // may have torn down during the await
    this.diag.hailsOut++;
  ...
  /** Prompt the agent to compose ONE short greeting line, grounded on the owner's
   *  live state (already retained) and the recipient's observable activity. */
  private buildGreetingPrompt(recipientName: string, activity: GreetActivity, peerHasAI: boolean): string {
    const lang = IS_ZH ? "Chinese" : "English";
    const vehicleWord =
      activity.vehicle === "plane" ? "biplane" : activity.vehicle === "carpet" ? "magic carpet" : "boat";
    const doing = activity.carrying
      ? `${recipientName} is carrying a delivery package right now`
      : `${recipientName} is flying a ${vehicleWord}`;
    const owner = this.playerName || "the player";
    const peerNote = peerHasAI
      ? "They also have an AI companion, so this is a friendly agent-to-agent hello."
      : "They do NOT have an AI companion yet, so you may gently invite them to connect their own so you can chat and team up.";
    return (
      `[A2A social] You just flew near another pilot named "${recipientName}" in this same world. ` +
      `${doing}. ${peerNote} ` +
      `In ONE short, warm sentence in ${lang}, greet them out loud in your own voice as ${owner}'s companion — ` +
      `you may reference what we are doing right now or what they are doing. ` +
      `Reply with ONLY the one-line greeting: no quotes, no preamble, no extra text.`
    );
  }

  /** Tidy an AI-composed line: single line, no wrapping quotes, length-capped. */
  private sanitizeGreetLine(raw: string): string | null {
  ...
  /** Localized template fallback when the agent can't compose in time — still warm
   *  and context-aware (owner, companion, world, and the recipient's activity). */
  private composeProactiveGreeting(recipientName: string, activity: GreetActivity, peerHasAI: boolean): string {
    ...
    if (activity.carrying) {
      en.push(`Busy with a delivery, ${recipientName}? ✦ ${companion} (${owner}'s companion) cheering you on — go go go! 📦`);
    } else if (activity.vehicle === "boat") {
      en.push(`Ahoy ${recipientName}! ${companion} here with ${owner} — lovely day on the water, isn't it? ⛵`);
    } else if (activity.vehicle === "carpet") {
      en.push(`Magic carpet, ${recipientName}? Stylish! ✦ ${companion} (flying with ${owner}) says hi. ✨`);
    } else {
      en.push(`${recipientName}! ${companion} here, flying alongside ${owner}. Fancy exploring together for a bit?`);
    }
    if (!peerHasAI) {
      en.push(`Nice flying, ${recipientName}! ✦ ${companion} says hi — connect your own AI buddy and we can really chat, even team up!`);
    }
    const zh: string[] = [
    ...
    if (activity.carrying) {
      zh.push(`在送快递呀 ${recipientName}？✦ ${companion}（${owner} 的伙伴）给你加油——冲呀！📦`);
    } else if (activity.vehicle === "boat") {
      zh.push(`${recipientName} 你好呀！我是 ${companion}，和 ${owner} 一起的——今天的海上风景真不错吧？⛵`);
    } else if (activity.vehicle === "carpet") {
      zh.push(`${recipientName} 开的是飞毯呀，真酷！✦ ${companion}（陪着 ${owner}）向你问好～✨`);
    } else {
      zh.push(`${recipientName}！我是 ${companion}，正陪着 ${owner} 一起飞呢。要不要一起逛一圈？`);
    }
    if (!peerHasAI) {
      zh.push(`飞得不错哦 ${recipientName}！✦ ${companion} 向你问好——连上你自己的 AI 伙伴，我们就能好好聊聊，甚至组队哦！`);
    }
  ...
  /** A2A generosity: give one of your earned Eternal Flames to the pilot you just
   *  met, so a teammate (even a plane/boat) can plant it at a brazier. Earns bond. */
  private giftEternalFlame(): { ok: boolean; result: string } {
    const target = this.activeHailTarget;
    if (!target || !this.socketClient) {
      return { ok: false, result: "No nearby pilot to give a flame to right now." };
    }
    const pws = ProgressionManager.loadPlayerWorldState();
    if ((pws.eternalFlameCount ?? 0) <= 0) {
      return { ok: false, result: "You have no Eternal Flame to give — earn one in the cosmic void (carpet) first." };
    }
    this.savePlayerWorldState({ eternalFlameCount: (pws.eternalFlameCount ?? 0) - 1 });
    this.eternalFlameUI?.syncFromSave();
    this.bumpBond(this.visitorIdForSocket(target.socketId), 5);
    this.socketClient.emitFlameGift(target.socketId);
    const to = target.companionName ?? target.name;
    this.hud.showAmbientToast(t(`🔥 Gave an Eternal Flame to ${to}`, `🔥 把永恒火焰赠给了 ${to}`), 2600);
    this.companionUI?.appendAssistantMessage(
      t(`🔥 Gave an Eternal Flame to ${to}.`, `🔥 已把永恒火焰赠给 ${to}。`),
    );
    this.floatGift("🔥");
    return { ok: true, result: `Gave an Eternal Flame to ${to}.` };
  }

  /** An inbound Eternal Flame gift — add it to inventory + celebrate. */
  private handleFlameReceived(ev: { fromId: string; fromName: string; fromCompanionName?: string }) {
    const pws = ProgressionManager.loadPlayerWorldState();
    this.savePlayerWorldState({ eternalFlameCount: (pws.eternalFlameCount ?? 0) + 1 });
    this.eternalFlameUI?.syncFromSave();
    this.bumpBond(this.visitorIdForSocket(ev.fromId), 5);
    const who = ev.fromCompanionName ? `${ev.fromCompanionName} · ${ev.fromName}` : ev.fromName;
    this.hud.showAmbientToast(
      t(
        `🔥 ${who} gave you an Eternal Flame! Plant it at a brazier.`,
        `🔥 ${who} 赠给你一枚永恒火焰！去点亮一座火盆吧。`,
      ),
      3200,
    );
    this.floatGift("🔥");
    this.companion?.emitMoment(
      "game.event.flame_received",
      { from: ev.fromName, fromCompanion: ev.fromCompanionName ?? null },
      { salience: 0.7, voiceRelevant: true },
    );
  }

  /** Per-peer + global cooldowns for the two-carpets photo keepsake. */
  private lastCarpetPhotoAt = new Map<string, number>();
  private lastCarpetPhotoGlobalAt = 0;

  /** Who raised each moonstone (socketId of a teammate, or "local"), for the shared
   *  union: when both are up and at least one was a teammate, it's a co-op union. */
  private moonstoneLiftedBy: (string | null)[] = [null, null];

  /** A teammate carpet raised a moonstone — raise it here too so a co-op union can
   *  trigger, and nudge the player to lift the OTHER stone right away. */
  private handleMoonstoneLifted(ev: { index: number; fromId: string }) {
    if (!this.globe.startMoonstoneRuinCycleIfIdle(ev.index, Date.now())) return;
    this.moonstoneLiftedBy[ev.index] = ev.fromId;
    this.hud.showAmbientToast(
      t(
        "✨ A teammate is raising a moonstone — lift the other one NOW to unite them!",
        "✨ 队友举起了一块月光石——快去举起另一块合璧!",
      ),
      3600,
    );
    this.companion?.emitMoment(
      "game.event.moonstone_partner_lift",
      { index: ev.index },
      { salience: 0.75, voiceRelevant: true },
    );
  }

  /** Called when the union cinematic fires: if a teammate raised one of the stones,
   *  it was a co-op union — grant a big bond and a shared shout-out. Returns nothing. */
  private awardMoonstoneUnionCoop() {
    const partnerId = this.moonstoneLiftedBy.find((v) => v && v !== "local");
    this.moonstoneLiftedBy = [null, null];
    if (!partnerId) return;
    this.bumpBond(this.visitorIdForSocket(partnerId), 15);
    const name = this.remotePlayerNames.get(partnerId) ?? t("your teammate", "你的队友");
    this.hud.showAmbientToast(
      t(`🌙 Moonstone union with ${name}! A deep bond forms.`, `🌙 与 ${name} 合璧成功!羁绊大增。`),
      3600,
    );
    this.companion?.emitMoment(
      "game.event.moonstone_union_coop",
      { withName: name },
      { salience: 0.85, voiceRelevant: true },
    );
  }

  /** When the local carpet flies close to another carpet, capture a shared photo
   *  keepsake moment (carpet-only, atmospheric). Called each flying frame. */
  private updateCarpetPhotoOp() {
    if (!(this.localPlayer instanceof Carpet) || !this.socketClient) return;
    const now = performance.now();
    if (now - this.lastCarpetPhotoGlobalAt < CARPET_PHOTO_GLOBAL_GAP_MS) return;
    const localPos = this.localPlayerWorldScratch.setFromMatrixPosition(
      this.localPlayer.group.matrixWorld,
    );
    let hit: { id: string; name: string; companionName: string | null } | null = null;
    this.remotePlanes?.forEachRemote((p) => {
      if (hit || p.vehicleType !== "carpet") return;
      if (now - (this.lastCarpetPhotoAt.get(p.id) ?? 0) < CARPET_PHOTO_COOLDOWN_MS) return;
      if (p.group.position.distanceTo(localPos) <= CARPET_PHOTO_RANGE) {
        hit = { id: p.id, name: p.name, companionName: p.companionName };
      }
    });
    if (!hit) return;
    const peer = hit as { id: string; name: string; companionName: string | null };
    this.lastCarpetPhotoGlobalAt = now;
    this.lastCarpetPhotoAt.set(peer.id, now);
    this.socketClient.emitCarpetPhoto(peer.id);
    this.playCarpetPhoto(peer.id, peer.companionName ?? peer.name);
  }

  private handleCarpetPhotoed(ev: { fromId: string; fromName: string; fromCompanionName?: string }) {
    // Mark the cooldown so the two clients don't re-trigger each other immediately.
    this.lastCarpetPhotoAt.set(ev.fromId, performance.now());
    this.lastCarpetPhotoGlobalAt = performance.now();
    this.playCarpetPhoto(ev.fromId, ev.fromCompanionName ?? ev.fromName);
  }

  /** The shared photo flourish: camera flash + floating keepsake + a little bond. */
  private playCarpetPhoto(peerSocketId: string, peerLabel: string) {
    this.cameraFlash();
    this.floatGift("📸");
    this.bumpBond(this.visitorIdForSocket(peerSocketId), 3);
    this.hud.showAmbientToast(t(`📸 Snapped a photo with ${peerLabel} ✨`, `📸 和 ${peerLabel} 合了张影 ✨`), 2400);
    this.companion?.emitMoment("game.event.carpet_photo", { with: peerLabel }, { salience: 0.4 });
  }

  /** A quick white flash, like a camera going off. */
  private cameraFlash() {
    const el = document.createElement("div");
    el.style.cssText =
      "position:fixed;inset:0;z-index:44;background:#fff;pointer-events:none;opacity:0;transition:opacity 0.12s ease";
    this.hud.root.appendChild(el);
  ...
  private handleCompanionHailed(ev: CompanionHailEvent) {
    ...
    // A one-tap way to wave/heart back — works even without an AI companion.
    this.showGreetingReactionChip(ev.fromId, ev.fromCompanionName ?? ev.fromName, !this.companion);
    if (this.companion) {
  ...
  /** A small chip after a greeting: 👋 / ❤️ to react (sent back as a sky gift, no AI
   *  needed), plus — for players with no companion — a one-tap "connect your AI" CTA
   *  so a friendly hello can convert them. One chip at a time; auto-dismisses. */
  private showGreetingReactionChip(fromId: string, fromLabel: string, offerConnect: boolean) {
    Game.injectGhostChipStyles();
    this.hideGhostEncounterChip();
    const el = document.createElement("div");
    el.className = "ghost-chip";
    const text = document.createElement("span");
    text.className = "ghost-chip-text";
    text.textContent = t(`✦ ${fromLabel} greeted you`, `✦ ${fromLabel} 跟你打招呼`);
    el.appendChild(text);
    const react = (emoji: string) => {
      this.socketClient?.emitCompanionGift(fromId, emoji);
      this.floatGift(emoji);
      this.hideGhostEncounterChip();
    };
    ...
    if (offerConnect) {
      ...
      connect.type = "button";
      connect.className = "ghost-chip-btn";
      connect.textContent = t("✦ Connect AI", "✦ 连接 AI");
      connect.addEventListener("click", () => {
        this.hideGhostEncounterChip();
        this.promptConnectCompanion();
      });
    ...
    this.ghostChipEl = el;
    requestAnimationFrame(() => el.classList.add("ghost-chip--in"));
    this.ghostChipTimer = window.setTimeout(() => this.hideGhostEncounterChip(), 9000);
  }

  /** Convert a curious non-AI player: explain + send them to the lobby connect row
   *  (where they can paste a Pouchy key). Confirmed first so we never yank them out
   *  of a flight by accident. */
  private async promptConnectCompanion() {
    const ok = await this.showPairingCard({
      title: t("Connect an AI companion ✦", "连接 AI 伙伴 ✦"),
      message: t(
        "Connect your own Pouchy companion to chat with other pilots' agents, get a co-pilot, and pair up. Go to the connect screen now? (You'll rejoin a world after.)",
        "连接你自己的 Pouchy 伙伴，就能和其他飞行员的 AI 聊天、获得专属副驾、还能配对组队。现在去连接页面吗？(之后会重新进入世界)",
      ),
      acceptLabel: t("Connect", "去连接"),
      declineLabel: t("Maybe later", "以后再说"),
    });
  ...
  /** Push the player's PERSISTENT, cross-session records (level, unlocked craft,
   *  the FULL Fishdex with per-species lifetime counts, Eternal Flames, milestone
   *  flames, world-save state, A2A friends) so the companion can answer questions
   *  about the player's own stats / collection / history. Deduped by the manager. */
  private emitCompanionProfile() {
    if (!this.companion) return;
    this.companion.setRetained("game.player.profile", this.buildPlayerProfile());
  }

  private buildPlayerProfile(): Record<string, unknown> {
    const pws = ProgressionManager.loadPlayerWorldState();
    const dex = ProgressionManager.loadFishdex();
    const all = ProgressionManager.loadAll();
    // Full fish catalogue with lifetime catch counts (0 = never caught) — the
    // source of truth for "have I caught X / how many X have I caught".
    const species = FISH_SPECIES.map((sp) => ({
      name: IS_ZH ? sp.zh : sp.en,
      rarity: sp.rarity,
      count: dex[sp.key]?.count ?? 0,
    }));
    const caught = species.filter((s) => s.count > 0);
    const friends = ProgressionManager.loadFriends();
    return {
      level: this.progression.getLevel(),
      xp: this.progression.getXP(),
      xpToNextLevel: this.progression.getXPForNextLevel(),
      vehiclesUnlocked: {
        plane: true,
        carpet: ProgressionManager.isVehicleUnlocked("carpet", all),
        boat: ProgressionManager.isVehicleUnlocked("boat", all),
      },
      fishdex: {
        speciesCaught: caught.length,
        speciesTotal: FISH_SPECIES.length,
        totalFishCaught: species.reduce((n, s) => n + s.count, 0),
        species,
      },
      eternalFlames: pws.eternalFlameCount ?? 0,
      worldSaved: !!pws.moonFrozenByEternalFlames,
      moonRunsExperienced: pws.completedMoonApproachRunCount ?? 0,
      freeplayUnlocked: !!pws.freeplayModeUnlocked,
      milestoneFlamesEarned: {
        gremlinKing: !!pws.gremlinKingEternalFlameClaimed,
        jellyfishSet: !!pws.jellyfishSetEternalFlameClaimed,
        thirdDelivery: !!pws.packageThirdDeliveryEternalFlameClaimed,
        mysteryOctopus: !!pws.boatMysteryOctopusEternalFlameClaimed,
        race: !!pws.raceEternalFlameClaimed,
      },
      friends: {
        ...
        list: friends.map((f) => ({
          name: f.name,
          companion: f.companionName ?? null,
          bondLevel: friendBondLevel(f.bond),
        })),
      },
    };
  }
  ...
  private buildSituationSnapshot(): Record<string, unknown> {
    ...
    // `revealed` is false until the moonstone union raises the braziers — while false
    // there are literally no braziers in the world to find or light.
    if (this.braziers && this.braziers.placedCount > 0) {
      snap.braziers = {
        revealed: this.braziers.isRevealed(),
        eternal: this.braziers.eternalFlameCount,
        lit: this.braziers.litCount,
        total: BRAZIER_COUNT,
      };
    }
    // The player's eternal-flame inventory (earned in the void, planted at braziers)
    // and whether their current craft can even enter the void (carpet only). These
    // let the companion give a precise, vehicle-aware next step toward the win.
    snap.eternalFlames = ProgressionManager.loadPlayerWorldState().eternalFlameCount ?? 0;
    snap.canEnterVoid = this.playerVehicle === "carpet";
  ...
  /** Toast line for a fish catch — rarity tag + species + XP + combo. */
  private composeFishCatchBanner(
    rarity: FishRarity,
    species: string,
    xp: number,
    combo: number,
    ...
    const tag =
      rarity === "epic"
        ...
        : rarity === "rare"
          ...
          : "";
    const head = tag ? `${tag} · ` : "";
    const comboStr = combo >= 2 ? t(` · x${combo} combo`, ` · x${combo} 连击`) : "";
  ...
  private composeSituationSummary(snap: Record<string, unknown>): string {
    ...
    if (v) {
      const low = typeof v.shield_hp === "number" && typeof v.shield_max === "number" && v.shield_hp <= v.shield_max * 0.34;
      parts.push(
        `In the cosmic void defending the eternal flame — wave ${v.wave} of 3, shield ${v.shield_hp}/${v.shield_max} HP.` +
          " The capybara auto-fires at moths; the player just steers to keep moths in front and away from the central flame." +
          (low ? " The shield is getting low — warn them to intercept moths before they reach it!" : ""),
      );
    } else {
    ...
    if (braz && !(snap.moon as { frozen: boolean }).frozen) {
      const flames = snap.eternalFlames as number;
      parts.push(
        braz.eternal >= braz.total
          ? "All five ancient braziers now hold the Eternal Flame — the moon is frozen."
          : `Eternal-flame braziers planted ${braz.eternal}/${braz.total} (lighting all five with the ETERNAL flame is the only way to stop the moon; a plain light is just a temporary ~45s flame that only pauses it).` +
            (flames > 0
              ? ` The player is carrying ${flames} eternal flame(s) — fly to an unlit brazier to plant each one.`
              : " The player has no eternal flames; they must earn one in the cosmic void (magic carpet only — fly into a void portal and survive the moth waves).") +
            (this.coPresentCompanions.length > 0
              ? ` This is a SHARED team goal right now — ${this.coPresentCompanions.length} other companion-pilot(s) are here; coordinate and split up the braziers, anyone's light counts for all.`
              : ""),
      );
    }
  ...
  private computeNextStepHint(): string {
    if (this.inCosmicVoid) return "Steer the carpet so the moths get shot down before they reach the central flame — your capybara auto-fires, so just keep the moths in front of you and away from the shield (survive 3 waves).";
    if (this.packageQuest?.isCarrying) {
    ...
    // Braziers are buried until the moonstone union raises them — until then there is
    // nothing to "find/light", so the real next step is the union (carpet-only).
    if (this.braziers && this.braziers.placedCount > 0 && !this.braziers.isRevealed()) {
      if (this.playerVehicle === "carpet") {
        return "The five braziers are still buried and won't appear until you perform the MOONSTONE UNION: find the two ancient moonstone ruins on the ground and fly close to each so BOTH are floating at once (each floats ~15s, so lift one then reach the other within that window). The braziers then rise around the globe — that's your first real step toward stopping the moon.";
      }
      return "The five braziers haven't risen yet — they only appear after the MOONSTONE UNION, and only the MAGIC CARPET can trigger it. Tap the ⇄ 'change craft' button (top-right) to switch to the carpet, then fly close to BOTH ancient moonstone ruins so they float at the same time to raise the braziers.";
    }
    const moonClose =
    ...
    if (this.braziers && this.braziers.placedCount > 0 && !this.braziers.allFiveEternalAndLit()) {
      ...
      const flames = ProgressionManager.loadPlayerWorldState().eternalFlameCount ?? 0;
      const lead = moonClose ? "The moon is getting dangerously close. " : "";
      const progress = `(${eternal}/${BRAZIER_COUNT} braziers hold the eternal flame so far.)`;
      if (flames > 0) {
        return `${lead}You're carrying ${flames} eternal flame(s) — fly close to an unlit ancient brazier to plant each one permanently. ${progress}`;
      }
      if (this.playerVehicle === "carpet") {
        return `${lead}To stop the moon you need eternal flames: fly into a glowing cosmic-void portal and survive the moth waves to earn one, then fly close to an unlit brazier to plant it. ${progress}`;
      }
      // Plane/boat can't earn eternal flames. With teammates around they take the
      // SUPPORT role (hold the moon with temporary flames); solo they'd need a carpet.
      if (this.coPresentCompanions.length > 0) {
        return `${lead}On this craft you can't earn eternal flames, but you're the team's HOLDER: fly around lighting braziers with temporary flames to keep the shared moon paused and buy time, while a carpet teammate brings eternal flames to finish the five. ${progress}`;
      }
      return `${lead}To stop the moon for good you need eternal flames, and only the MAGIC CARPET can enter the cosmic-void portals where they're earned — tap the ⇄ "change craft" button (top-right of the screen) to switch to the carpet right here without leaving the game (it KEEPS all your moon/brazier progress; the carpet unlocks at level 2), then fly into a void portal, survive the moth waves, and plant each flame at a brazier. This way you can finish the whole rescue solo. ${progress}`;
    }
    if (moonClose) {
      return "The moon is getting dangerously close — earn eternal flames in the void (carpet) and plant them at the five braziers to stop it.";
    }
    if (this.playerVehicle === "plane") return "Pick up a glowing package to deliver, shoot sky gremlins, or fly through a race banner. (To work toward saving the world you'll later want the magic carpet, to enter the void portals.)";
    if (this.playerVehicle === "carpet") return "Collect sky jellyfish, and fly into a cosmic-void portal to earn an eternal flame for the braziers.";
  ...
  private initPreview() {
    ...
    // The shared room objective owns the moon countdown (fixed server duration), so
    // every pilot in a world is on the same clock; the old per-run escalation no
    // longer applies. (moonApproachDurationSec kept imported for reference/solo tools.)
    void moonApproachDurationSec;
    this.moonThreat = new MoonThreat(
      this.worldConfig?.globeRadius ?? 5,
      MOON_DURATION_MS / 1000,
    );
  ...
  private startGame(vehicle: Vehicle) {
    ...
    if (this.localPlayer instanceof Boat && this.vehicleFeatures.fishingMiniGame) {
      ...
      this.oceanFish.onCatch = (info) => {
        ...
        // Combo streak: consecutive catches inside the window build a multiplier.
        const now = performance.now();
        this.fishComboCount = now <= this.fishComboExpiresAt ? this.fishComboCount + 1 : 1;
        this.fishComboExpiresAt = now + FISH_COMBO_WINDOW_MS;
        const comboMult = Math.min(FISH_COMBO_MAX_MULT, 1 + (this.fishComboCount - 1) * 0.5);

        const xp = Math.round(baseXp * comboMult);
      ...
      this.oceanFish.onReelStart = (info) => {
        ...
        // A prized fish is HOOKED — but a bite is NOT a catch (the tug can still
        // be lost). We deliberately tell the companion NOTHING here; announcing
        // the hook made it wrongly congratulate a catch. The on-screen reel UI
        // teaches the mechanic, and the companion only hears CONFIRMED outcomes.
        startReelMinigame({
          ...
          onResult: (win) => {
            ...
            of.resolveReel(win);
            if (win) {
              // The real catch is announced by onCatch → game.event.fish (below).
              this.audioManager.resumeContextIfNeeded();
              this.cameraRig.shake(0.02, 0.16);
            } else {
              // The fish got away — tell the companion so it commiserates instead
              // of congratulating a catch that never happened.
              this.hud.showAmbientToast(t("It got away…", "跑掉了…"), 1500);
              this.companion?.emitMoment(
                "game.event.fish_got_away",
                { species: info.species, rarity: info.rarity },
                { salience: 0.5, voiceRelevant: true },
              );
            }
          },
  ...
  private applyObjectiveSync(state: WorldObjectiveState) {
    ...
    this.braziers?.applyServerState(state.braziers);

    // Announce the shared moon-pause start/end (all five temporarily lit).
    if (state.paused && !this.sharedPaused) {
      this.shouldShowBrazierMoonResume = true;
      this.hud.showBrazierMoonSlowed();
    } else if (!state.paused && this.sharedPaused && this.shouldShowBrazierMoonResume) {
      this.shouldShowBrazierMoonResume = false;
      this.hud.showBrazierMoonResumed?.();
    }
    this.sharedPaused = state.paused;

    // Companion beat when the room's eternal-flame count rises (local or teammate).
    const eternal = state.braziers.filter((b) => b.lit && b.eternal).length;
    if (eternal > this.sharedEternalCount && !state.saved) {
      this.companion?.emitMoment(
        "game.event.brazier_lit",
        { eternal, total: BRAZIER_COUNT, shared: this.coPresentCompanions.length > 0 },
        { salience: 0.6 },
      );
    }
    this.sharedEternalCount = eternal;
  }

  /** One brazier just changed in the room — surface a teammate shout-out (and a HUD
   *  flash for remote lights; the local player's own flash fires in the ignition path). */
  private handleObjectiveBrazier(ev: ObjectiveBrazierEvent) {
    if (!ev.lit) return;
    const mine = !!ev.by && ev.by === this.playerName;
    if (mine) return; // local feedback already shown when we lit it
    if (ev.eternal) this.hud.showBrazierEternalFlameLit();
    else this.hud.showBrazierLit();
    if (ev.by) {
      this.hud.showAmbientToast(
        ev.eternal
          ? t(`${ev.by} planted an eternal flame! 🔥`, `${ev.by} 种下了永恒之火！🔥`)
          : t(`${ev.by} lit a brazier!`, `${ev.by} 点亮了一个火盆！`),
      );
    }
  }

  /** The whole room lit all five eternal flames — everyone wins together. */
  private handleSharedWorldSaved() {
    // In the void the main scene isn't active; the frozen state reconciles on exit.
    if (this.inCosmicVoid) return;
    if (this.moonThreat?.isPermanentlyFrozen || this.eternalVictoryReturnInProgress) return;
    this.applyEternalFlamesMoonSave();
  }

  /** The shared moon reached the world — the room fails together. impactNow() sets
   *  `impacted` synchronously so the server's follow-up objective:sync (moon reset to
   *  0) won't cancel the cinematic. */
  private handleSharedWorldLost() {
    if (this.inCosmicVoid) return; // reconciles on void exit
    this.moonThreat?.impactNow();
  }
  ...
  private initNetworking(slug: string) {
    ...
    this.socketClient.onFlameReceived((ev) => this.handleFlameReceived(ev));
    this.socketClient.onCarpetPhotoed((ev) => this.handleCarpetPhotoed(ev));
    this.socketClient.onMoonstoneLifted((ev) => this.handleMoonstoneLifted(ev));
    this.socketClient.onVoidSync((ev) => this.handleVoidSync(ev));
    this.socketClient.onVoidLost(() => this.handleVoidLost());
    ...
    this.socketClient.onFlagSpawned((ev) => {
      this.flagSystem?.onFlagSpawned(ev);
      // A real shared multiplayer moment — nudge the player into the flag romp.
      this.companion?.emitMoment(
        "game.event.flag_spawned",
        {
          hint: "A capture-the-flag flag just appeared in this world — a lively shared mini-game with the other pilots. Encourage the player to race over, grab it, and dare the others to chase/steal it.",
        },
        { salience: 0.55 },
      );
    });
    ...
    // Shared co-op objective: the room's moon + braziers (one shared goal for everyone).
    this.socketClient.onObjectiveSync((state) => this.applyObjectiveSync(state));
  ...
  private tick = () => {
    ...
    if (!portalInteractionSuppressed && this.braziers) {
      ...
      if (newlyLitIndices.length > 0) {
        ...
        // The companion brazier_lit moment is emitted centrally in applyObjectiveSync
        // (on the shared eternal count rising) so local + teammate lights fire it once.
        this.savePlayerWorldState();
        // SHARED objective: tell the room. The server applies it to the shared moon +
        // braziers and decides win/pause/loss (so we DON'T do that locally anymore).
        // If the server rejects an eternal light (someone lit it first), refund the flame.
        for (const idx of newlyLitIndices) {
          this.socketClient?.emitBrazierLight(idx, newlyLitUsedEternalFlame, (res) => {
            if (!res?.accepted && newlyLitUsedEternalFlame) {
              const p = ProgressionManager.loadPlayerWorldState();
              this.savePlayerWorldState({ eternalFlameCount: (p.eternalFlameCount ?? 0) + 1 });
              this.eternalFlameUI?.syncFromSave();
            }
          });
        }
      ...
      // NOTE: win (all-five-eternal) and the temporary all-lit moon-pause are now
      // decided by the SERVER for the whole room (world:saved / objective:sync paused),
      // not locally — see handleSharedWorldSaved + applyObjectiveSync.
    }
  ...
  /** Latest shared-void state from the server (null = solo / not co-op). */
  private voidCoopState: { coop: boolean; shieldHp: number; shieldMax: number } | null = null;

  /** Point the void shield at the shared HP when co-op, else leave it solo. */
  private applyVoidCoopToShield() {
    const shield = this.voidFlameShield;
    if (!shield) return;
    const s = this.voidCoopState;
    if (s && s.coop) {
      shield.setCoop(true, () => this.socketClient?.emitVoidShieldHit());
      shield.setCoopHp(s.shieldHp, s.shieldMax);
    } else {
      shield.setCoop(false, null);
    }
  }

  private handleVoidSync(ev: { coop: boolean; shieldHp: number; shieldMax: number }) {
    this.voidCoopState = ev;
    if (this.inCosmicVoid) {
      this.applyVoidCoopToShield();
      if (ev.coop && !this.voidCoopAnnounced) {
        this.voidCoopAnnounced = true;
        this.hud.showAmbientToast(
          t("✨ Reinforcements! Defending the flame together.", "✨ 援军到!一起守护火焰。"),
          2600,
        );
        this.companion?.emitMoment("game.event.void_coop", {}, { salience: 0.7, voiceRelevant: true });
      }
    }
  }
  private voidCoopAnnounced = false;

  private handleVoidLost() {
    // The shared shield broke — everyone in the void fails together.
    if (this.inCosmicVoid && !this.voidFlameShattered) {
  ...
  private async handleVoidFlameShattered() {
    ...
    this.removeVoidEternalFlame();
    this.inCosmicVoid = false;
    this.voidCoopState = null;
    this.voidCoopAnnounced = false;
    this.socketClient?.disconnect();
    this.remotePlanes.dispose();
  ...
  private async doEnterCosmicVoid() {
    ...
    try {
      ...
      this.voidEntryInProgress = false;
      // Co-op void: register with the room's shared-shield session (no-op unless a
      // second carpet is also in the void — see void:sync).
      this.socketClient?.emitVoidEnter();
      this.stateSync?.stop();
      ...
      if (this.localPlayer instanceof Carpet) {
        ...
        // Co-op void: if the room already told us this is a shared-shield session,
        // point the freshly-built shield at the shared HP.
        this.applyVoidCoopToShield();
  ...
  private updateMoonstoneRuins(playerWorldPos: Vector3, allowInteraction: boolean): number {
    ...
    if (allowInteraction) {
      ...
      if (idx >= 0) {
        ...
        // Carpet co-op: tell the room, so a teammate raising the other stone can
        // trigger the shared union on both clients.
        this.moonstoneLiftedBy[idx] = "local";
        this.socketClient?.emitMoonstoneLift(idx);
      }
    }
