...
const METEOR_PHASE_START = 0.85;
const METEOR_PHASE_END = 1.0;
const METEOR_POOL = 4;
const METEOR_MAX_ACTIVE = 3;

const METEOR_SPAWN_INTERVAL_MAX = 1.9;
const METEOR_SPAWN_INTERVAL_MIN = 0.9;
const METEOR_TRAVEL_TIME_MIN = 2.4;
const METEOR_TRAVEL_TIME_MAX = 3.4;

const METEOR_TARGET_SURFACE_DIST_MIN = 1.6;
const METEOR_TARGET_SURFACE_DIST_MAX = 3.1;
const METEOR_TARGET_HEADING_SPREAD = 1.9;

const METEOR_SPAWN_HEIGHT_MIN = 3.5;
const METEOR_SPAWN_HEIGHT_MAX = 5.0;
const METEOR_SIDE_SWAY = 0.55;
const METEOR_FORWARD_SWAY = 0.35;

const METEOR_HEAD_RADIUS = 0.085;
const METEOR_TRAIL_RADIUS = 0.09;
...
const IMPACT_DOME_LIFE = 0.70;
const IMPACT_SHOCKWAVE_LIFE = 0.84;
...
void main() {
  ...
  vec3 fireCol = mix(vec3(1.0, 0.2, 0.0), vec3(1.0, 0.9, 0.3), core);
  vec3 smokeCol = vec3(0.08, 0.08, 0.08);
  
  float fireMix = smoothstep(0.3, 0.8, fade + n * 0.2);
  vec3 col = mix(smokeCol, fireCol, fireMix);
  
  float alpha = smoothstep(0.0, 0.2, fade) * n * uOpacity;
  alpha = mix(alpha * 1.5, alpha, fireMix);
...
export class MeteorShower {
  ...
  private emitSpark(pos: Vector3, normal: Vector3, speedMult = 1.0) {
    ...
    spark.vel.normalize().multiplyScalar((0.25 + Math.random() * 0.75) * speedMult);
    
    spark.maxLife = 0.8 + Math.random() * 1.2;
  ...
  update(
    ...
    for (const meteor of this.meteors) {
      ...
      if (!meteor.exploding) {
        ...
        const fadeIn = MathUtils.clamp(rawT / 0.15, 0, 1);
        ...
        meteor.headMat.uniforms.uOpacity.value = fadeIn;
        ...
        meteor.trailMat.uniforms.uOpacity.value = MathUtils.lerp(0.4, 0.95, 1 - rawT) * fadeIn;

        // Emit sparks while flying
        if (Math.random() < dt * 30 * fadeIn) {
        ...
        meteor.glowMat.opacity = MathUtils.lerp(0.01, 0.05, rawT) * glowPulse;
    ...
    for (let i = 0; i < SPARK_COUNT; i++) {
      ...
      spark.vel.multiplyScalar(0.975); // drag
